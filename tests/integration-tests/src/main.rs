...
use flux_engine_core::{EnvelopeScanner, FlowOutcome, FlowState, IngestionOutcome, PacketView, FlowMap, FlowKey, LogicalByteView};
use flux_engine_core::scratchpad::{ForensicScratchpadPool, ScratchpadTier, ScratchpadGuard};
...
async fn set_flow_state(world: &mut EngineWorld, state: String) {
    ...
    world.flow_map.process_packet(&key, 0x02, 0, &[], 1000, world.scratchpad_pool);
    if state == "SynAckSeen" || state == "EstablishedTracking" || state == "ClientHelloIncomplete" {
        world.flow_map.process_packet(&key, 0x12, 0, &[], 2000, world.scratchpad_pool);
    }
    if state == "EstablishedTracking" || state == "ClientHelloIncomplete" {
        world.flow_map.process_packet(&key, 0x10, 0, &[], 3000, world.scratchpad_pool);
    }
...
async fn ingest_syn(world: &mut EngineWorld) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x02, 0, &[], 1000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn ingest_syn_ack(world: &mut EngineWorld) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x12, 0, &[], 2000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn ingest_ack(world: &mut EngineWorld) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, 0, &[], 3000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn ingest_rst(world: &mut EngineWorld) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x04, 0, &[], 4000, world.scratchpad_pool);
...
async fn ingest_partial_hello(world: &mut EngineWorld) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, 1000, b"HELL", 5000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn ingest_final_hello(world: &mut EngineWorld) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, 1004, b"O", 6000, world.scratchpad_pool);
...
async fn ingest_seq_packet(world: &mut EngineWorld, seq: i32, len: i32) {
    ...
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, seq as u32, &data, 1000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn ingest_ooo_packet_regex(world: &mut EngineWorld) {
    ...
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, 951, &data, 1000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn init_overlap_flow(world: &mut EngineWorld, state: String, len: i32, seq: i32, content: String) {
    set_flow_state(world, state).await;
    let key = EngineWorld::get_key();
    let data = vec![content.as_bytes()[0]; len as usize];
    world.flow_map.process_packet(&key, 0x10, seq as u32, &data, 1000, world.scratchpad_pool);
}
...
async fn verify_fww(world: &mut EngineWorld, start: i32, _end: i32, expected: String) {
    let key = EngineWorld::get_key();
    let entry = world.flow_map.acquire(&key, 1000).unwrap();
    let rb = entry.reassembly.as_ref().unwrap();
    let offset = (start as u32).wrapping_sub(rb.base_seq) as usize;
    let mut buf = [0u8; 1];
    assert_eq!(rb.copy_to(offset, &mut buf), 1);
...
async fn init_full_map(world: &mut EngineWorld) {
    for i in 0..700 { 
        let key = FlowKey { src_ip: IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1)), dst_ip: IpAddr::V4(Ipv4Addr::new(10, 0, 0, 2)), src_port: i, dst_port: 80, protocol: 6 };
...
async fn ingest_probe_overflow(world: &mut EngineWorld, _limit: i32) {
    for i in 0..2000 {
        let key = FlowKey { src_ip: IpAddr::V4(Ipv4Addr::new(10, 1, 0, 1)), dst_ip: IpAddr::V4(Ipv4Addr::new(10, 0, 0, 2)), src_port: i, dst_port: 80, protocol: 6 };
...
async fn init_full_pool(world: &mut EngineWorld) {
    while let Some(guard) = world.scratchpad_pool.acquire(ScratchpadTier::Tier1) {
        let static_guard: ScratchpadGuard<'static> = unsafe { std::mem::transmute(guard) };
        world.held_guards.push(static_guard);
    }
}
...
async fn ingest_heavy_payload(world: &mut EngineWorld) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, 1000, b"DATA", 5000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn init_frag_flow(world: &mut EngineWorld, state: String, count: i32) {
    ...
    let key = EngineWorld::get_key();
    for i in 0..count {
        world.flow_map.process_packet(&key, 0x10, 1000 + (i as u32 * 100), b"A", 1000, world.scratchpad_pool);
    }
}
...
async fn ingest_extra_frag(world: &mut EngineWorld, _count: i32) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, 9999, b"B", 1000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn ingest_win_overflow(world: &mut EngineWorld, _blocks: i32) {
    let key = EngineWorld::get_key();
    world.last_flow_outcome = world.flow_map.process_packet(&key, 0x10, 1000 + (70 * 1024), b"DATA", 1000, world.scratchpad_pool);
    world.current_flow_state = world.flow_map.get_state(&key);
}
...
async fn advance_clock(world: &mut EngineWorld, ms: i32) {
    let outcomes = world.flow_map.cleanup_expired(1000 + (ms as u64 * 1_000_000));
...
async fn verify_init_failure(world: &mut EngineWorld) {
    assert_eq!(world.last_flow_outcome, Some(FlowOutcome::UnsupportedTimingSource));
}

#[tokio::main]
async fn main() {
    let feature_path = if std::path::Path::new("tests/features").exists() { "tests/features" } else { "../../tests/features" };
